...
import { CallBackFormType } from "./formInput/CallBackFormType";
...
const CallBackForm: React.FC<LeadFormProps> = (props: any) => {
  const data = props.data;
  const formType = props.formType;
  const [loading, setLoading] = useState(true);
  const [submitting, setSubmitting] = useState(false);
  const [formField, setFormField] = useState<any>(
    data?.id ? populateFormFields(CallBackFormType, data) : CallBackFormType
  );

  const [formData, setFormData] = useState<any>(
    data?.id ? populateFormData(CallBackFormType, data) : {}
  );
