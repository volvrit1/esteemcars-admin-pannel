...
import { NewsletterType } from "./formInput/NewsletterType";
import {
  populateFormFields,
  populateFormData,
...
const NewsLetterForm: React.FC<NewsletterFormProps> = (props: any) => {
  const data = props.data;
  const formType = props.formType;
  const [loading, setLoading] = useState(true);
  const [submitting, setSubmitting] = useState(false);
  const [formField, setFormField] = useState<any>(
    data?.id ? populateFormFields(NewsletterType, data) : NewsletterType
  );

  const [formData, setFormData] = useState<any>(
    data?.id ? populateFormData(NewsletterType, data) : {}
  );
