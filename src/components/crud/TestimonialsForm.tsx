...
import { TestimonialsType } from "./formInput/TestimonialsFormType";
...
const TestimonialsForm: React.FC<TestimonialsFormProps> = (props: any) => {
  const data = props.data;
  const formType = props.formType;
  const [loading, setLoading] = useState(true);
  const [submitting, setSubmitting] = useState(false);
  const [formField, setFormField] = useState<any>(
    data?.id ? populateFormFields(TestimonialsType, data) : TestimonialsType
  );

  const [formData, setFormData] = useState<any>(
    data?.id ? populateFormData(TestimonialsType, data) : {}
  );
